...
export const comparisonEntries: ComparisonEntry[] = [
  {
    ...
    rows: [
      ...
      },
      {
        ...
        gitAgent: "yes — .git-agent/project.yml",
        competitor: "limited",
      },
    ],
    installComparison: {
...
Resolves multipart upload 500 error.`,
    ...
    faq: [
      ...
      },
      {
        ...
        answer: {
          en: "Migration requires no changes to git history. Run `git-agent init` to generate `.git-agent/project.yml` with your scopes and uninstall the Commitizen hook.",
          zh: "迁移不需要更改 git 历史。运行 `git-agent init` 生成包含范围的 `.git-agent/project.yml`，然后卸载 Commitizen 钩子即可。",
        },
      },
    ],
  },
    ...
    faq: [
      {
        ...
        answer: {
          en: "cz-git offers extensive TUI customisation options. git-agent is intentionally minimal in configuration — scope definitions in project.yml are the primary knob. More config reduces the automation value.",
          zh: "cz-git 提供广泛的 TUI 自定义选项。git-agent 在配置上有意保持简洁——project.yml 中的范围定义是主要配置项。过多配置会降低自动化的价值。",
        },
      },
      {
        question: {
