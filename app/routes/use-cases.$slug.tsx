...
export async function loader({ params }: LoaderFunctionArgs) {
	const { findUseCase } = await import("../data/use-cases");
	const entry = findUseCase(params.slug ?? "");
...
export const meta: MetaFunction<typeof loader> = ({ loaderData }) => {
	...
	return [
		...buildMeta({
			title: `${entry.title.en} — Git Agent Use Case | git-agent`,
			description: entry.description.en,
			canonicalUrl: `${BASE_URL}/use-cases/${entry.slug}`,
		}),
		{
	...
	return (
		<PseoLayout breadcrumb={`${t.pseoCrumbUseCases} / ${entry.slug}`} backHref="/use-cases">
			...
			<section className="command-section">
				<h2 className="section-label">{t.pseoUseCasesSectionChallenge}</h2>
				<p className="section-body">{renderInlineDocText(entry.challenge[language])}</p>
			</section>

			<section className="command-section">
				<h2 className="section-label">{t.pseoUseCasesSectionSolution}</h2>
				<p className="section-body">{renderInlineDocText(entry.solution[language])}</p>
			</section>

			<section className="command-section">
				<h2 className="section-label">{t.pseoUseCasesSectionSteps}</h2>
