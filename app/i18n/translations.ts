...
export const translations: Record<Language, Translations> = {
	en: {
		...
		homeTitle: "git-agent",
		homeSubtitle:
			"Git CLI for conventional commits and atomic splits — prefer official builds with FREE (no provider flags), then ~/.config/git-agent/config.yml; use --api-key/--base-url/--model only when you explicitly need overrides.",
		viewDetails: "View details →",

		// Entry cards
		initTitle: ["Initialize", "your repo"],
		initDescription:
			"Derive scopes from history, record a hook strategy in project config, and generate .gitignore in one command.",
		initFeatures: [
		...
		commitDescription:
			"Stage tracked changes, split into atomic commits, draft messages, and validate using hook_type (empty, conventional, or custom script).",
		commitFeatures: [
	...
	zh: {
		...
		homeTitle: "git-agent",
		homeSubtitle:
			"面向 Git 的 CLI：优先官方二进制 FREE（不带 provider 参数），其次 ~/.config/git-agent/config.yml；仅在明确需要时用 --api-key/--base-url/--model。",
		viewDetails: "查看详情 →",

		// Entry cards
		initTitle: ["初始化", "你的仓库"],
		initDescription:
			"从提交历史推导作用域，在项目配置中记录 hook 策略，并生成 .gitignore，一条命令完成。",
		initFeatures: [
			"从历史与目录推导作用域",
		...
		commitTitle: ["生成", "规范提交"],
		commitDescription:
			"暂存已跟踪的改动，拆成原子提交，起草消息，并按 hook_type 校验（empty、conventional 或自定义脚本）。",
		commitFeatures: [
			"原子提交拆分",
